provider "libvirt" {
  uri = "qemu+ssh://deploys@nebulia.shenron.dev/system?keyfile=~/.ssh/mdsstart"
}
...
resource "libvirt_domain" "domain-ubuntu" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      bastion_host        = "nebulia.shenron.dev"
      bastion_user        = "deploys"
      bastion_private_key = file("~/.ssh/mdsstart")
      timeout             = "2m"
    }
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo "ansible_ssh_common_args='-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand=\"ssh -W %h:%p -q nebulia.shenron.dev\"'" >> nginx.ini
